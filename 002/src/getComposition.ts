...
const replaceAllSubscript = (formula: string): string =>
  formula
    .replace(/\u2080/gu, "0")
    .replace(/\u2081/gu, "1")
    .replace(/\u2082/gu, "2")
...
const repeatFormula = (formula: string, multiplier: number): string => {
...
const unpack = (formula: string): string => {
...
const breakdown = (longFormula: string): Composition[] => {
  ...
  return Object.keys(counts).map((key) => {
    const element = elements.find((el) => el.symbol === key);
